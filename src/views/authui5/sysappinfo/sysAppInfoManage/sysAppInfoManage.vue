 ...
 * @LastEditTime: 2020-06-11 14:40:01
 * @LastEditors: Please set LastEditors
 -->
<template>
  <div class="frame-page">
    <z-search-panel>
      <z-form id="notifForm" name="notifForm" :label-width="90" label-position="left">
        <z-row :gutter="15">
    ...
    <z-list-panel class="m-t-15">
        ...
        <vxe-table
          ...
          <vxe-table-column field="appName" title="应用名称" width="120" sortable></vxe-table-column>
          <vxe-table-column field="appCode" title="应用代码"></vxe-table-column>
          <vxe-table-column field="appDomain" title="应用域名"></vxe-table-column>
          <vxe-table-column field="appPath" title="应用根地址"></vxe-table-column>
          <vxe-table-column field="appPathType" title="应用地址类型" :formatter="pathTypeStatus" width="120"></vxe-table-column>
          <vxe-table-column field="appIp" title="应用IP"></vxe-table-column>
...
import editSysInfo from '../editSysInfo/editSysInfo'
export default {
  ...
  methods: {
    ...
    add: function () {
      ...
      this.$refs.editSysInfo.handleReset()
    },
    edit: function (data) {
      this.modelType = 'edit'
      ...
      ref.editAppModel.appDomain = data.appDomain
      ref.editAppModel.appPath = data.appPath
      ref.editAppModel.appPathType = data.appPathType
      ref.editAppModel.appIp = data.appIp
    ...
    delBatch: function (data) {
      ...
      that.$Modal.confirm({
        ...
        onOk: function () {
          setTimeout(function () {
            sysAppApi.sysAppInfoDel(data.appId).then(function (response) {
    ...
    pathTypeStatus: function ({ cellValue, row, column }) {
      switch (cellValue) {
        case '1':
          return '默认类型'
        case '2':
