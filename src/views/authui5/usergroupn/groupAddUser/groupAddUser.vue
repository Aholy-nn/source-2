 ...
 * @LastEditTime: 2020-06-12 13:04:16
                ...
                <form-item :label="$t('userManage.title.orgName')">
                    <z-input  readonly v-model="orgTreeValue" @on-focus="drawerShow = true" clearable  @on-clear="orgClear"></z-input>
                        <z-drawer :title="$t('userManage.title.orgName')" :closable="false" v-model="drawerShow" transfer placement="left" width="300">
                            <z-vtree ref="groupOrgTree" :option="options" :node-icon="true"  @select="onSelectTree" v-model="treeValue" :toggleOnSelect="false"></z-vtree>
                        </z-drawer>
                </form-item>
            </z-col>
        </Row>
                ...
                <Row :gutter="10">
                    <z-col span="8">
                        <form-item :label="$t('userManage.title.userName')">
                            <z-input v-model="addModel.userName" clearable></z-input>
                        </form-item>
                    </z-col>
                    <z-col span="8">
                            <form-item :label="$t('userManage.title.userCode')">
                            <z-input v-model="addModel.userCode" clearable></z-input>
                        </form-item>
                    </z-col>
                    <z-col span="8">
        ...
        <card>
            ...
            <z-form ref="listModel" :model="listModel" :label-width="50" label-position="right">
                <Row :gutter="10">
                    <z-col span="8">
                        <form-item :label="$t('userManage.title.userName')">
                            <z-input v-model="listModel.userName" clearable></z-input>
                        </form-item>
                    </z-col>
                    <z-col span="8">
                            <form-item :label="$t('userManage.title.userCode')">
                            <z-input v-model="listModel.userCode" clearable></z-input>
                        </form-item>
                    </z-col>
                    <z-col span="8">
  ...
  methods: {
    getDatas: function (parent, resolve) {
      setTimeout(() => {
        ...
        userGroupAPI.getGroupOrgTree(params).then(function (response) {
    ...
    onSelectTree: function () {
      let selected = this.$refs['groupOrgTree'].getSelect()
      ...
      that.$Modal.confirm({
        ...
        onOk: function () {
          setTimeout(function () {
            ...
            userGroupAPI.changeBatch(data).then(function (response) {
    ...
    addQuery: function (type) {
      ...
      this.addModel.groupId = this.groupId
      if (type !== 'page') {
        this.addTablePage.page = 1
      }
      let params = Object.assign(this.addModel, this.addTablePage)
      userGroupAPI.groupUserWaitList(params).then(function (response) {
    ...
    //  已添加
    listQuery: function (type) {
      let that = this
      this.waitLoading = true
      this.listModel.groupId = this.groupId
      if (type !== 'page') {
        this.listTablePage.page = 1
      }
      let params = Object.assign(this.listModel, this.listTablePage)
      userGroupAPI.groupUserList(params).then(function (response) {
    ...
    handleSubmit: function () {
      ...
      let data = []
      let listTable = this.$refs['listTable']
      let insertRows = listTable.getInsertRecords()
      let removeRows = listTable.getRemoveRecords()
      if (insertRows.length !== 0) {
        for (let i = 0; i < insertRows.length; i++) {
          let obj = {
            groupId: this.groupId,
            userId: insertRows[i].userId,
            actionType: 'add'
          }
          data.push(obj)
        }
      ...
      userGroupAPI.changeBatch(data).then(function (response) {
        if (response.data.code === '01') {
          ...
          that.$Message.success(this.$t('common.messsage.success'))
          that.modelIsOpen = false
        } else {
          that.$Message.error(this.$t('common.messsage.fail'))
        }
      }).catch(function (error) {
        console.log(error)
      })
